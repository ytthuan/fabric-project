...
from flask import current_app as app, abort, session
import requests
import json
...
class PbiEmbedService:
    ...
    def get_embed_token_for_single_report_single_workspace(self, report_id, dataset_ids, target_workspace_id=None):
        ...
        # Effective Identity: include only when configured or dataset requires it
        include_identity = False
        rls_mode = app.config.get('RLS_MODE', 'auto').lower()
        requires_identity = False

        if rls_mode != 'off':
            # Try to detect dataset requirement
            try:
                for ds_id in dataset_ids:
                    if self._dataset_requires_effective_identity(ds_id, target_workspace_id):
                        requires_identity = True
                        break
            except Exception:
                # Fallback to not forcing identity on failures
                requires_identity = False

            include_identity = (rls_mode == 'on') or (rls_mode == 'auto' and requires_identity)
    ...
    def _dataset_requires_effective_identity(self, dataset_id, workspace_id):
        """Return True if the dataset requires EffectiveIdentity (RLS/roles), else False."""
        if not workspace_id:
            return False
        ds_url = f'https://api.powerbi.com/v1.0/myorg/groups/{workspace_id}/datasets/{dataset_id}'
        resp = requests.get(ds_url, headers=self.get_request_header())
        if resp.status_code != 200:
            return False
        ds = json.loads(resp.text)
        return bool(ds.get('isEffectiveIdentityRequired') or ds.get('isEffectiveIdentityRolesRequired'))
